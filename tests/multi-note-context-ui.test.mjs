...
test('the library tree exposes an explicit multi-select mode', () => {
  ...
  assert.match(appSource, /locale === 'zh' \? '多选文件' : 'Select multiple files'/);
  assert.doesNotMatch(appSource, /\{multiSelectActive \? t\('cancelMultiSelect'\) : t\('multiSelect'\)\}/);
  assert.match(appSource, /if \(multiSelectActive\) \{\s*onToggleContextFile\(entry\.relativePath, title\);/s);
...
test('selected files replace the implicit open page context for agent requests', () => {
  ...
  assert.match(appSource, /\$\{firstTitle\} 等 \$\{selectedContextFiles\.length\} 个文件/);
  assert.match(appSource, /currentPage=\{composerContextLabel\}/);
  assert.match(appSource, /currentPagePrimary=\{composerContextPrimaryLabel\}/);
  assert.match(appSource, /currentPageCount=\{composerContextCountLabel\}/);
  assert.match(appSource, /className="composer-context-title">\{currentPagePrimary \|\| currentPage\}<\/span>/);
  assert.match(appSource, /className="composer-context-count">\{currentPageCount\}<\/span>/);
  assert.match(cssSource, /\.composer-context-title\s*\{[^}]*flex:\s*1 1 auto;[^}]*text-overflow:\s*ellipsis;/s);
  assert.match(cssSource, /\.composer-context-count\s*\{[^}]*flex:\s*0 0 auto;[^}]*white-space:\s*nowrap;/s);
});

test('file context uses a removable pill and clearing it changes the next request', () => {
  ...
  assert.match(appSource, /const dismissComposerContext = \(\) => \{\s*setImplicitContextDismissed\(true\);\s*if \(selectedContextFiles\.length > 0\) cancelMultiSelect\(\);/s);
  ...
  assert.match(cssSource, /\.composer-context-title\s*\{[^}]*display:\s*block;[^}]*height:\s*16px;[^}]*overflow:\s*hidden;[^}]*text-overflow:\s*ellipsis;[^}]*white-space:\s*nowrap;/s);
