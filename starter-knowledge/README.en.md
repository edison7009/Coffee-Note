...
# Coffee Note Starter Knowledge Base

This is the independent, local-first knowledge base created when Coffee Note is launched for the first time. The starter content provides a reasonably complete foundation for reading and AI context, while containing no health parameters from the developer or any real user.
...
When the full Chinese or English title of another article appears in the body text, Coffee Note automatically displays the first occurrence as an internal link. A target may also be specified explicitly in Markdown:
...
The final part of the link uses the `id` from the target article's frontmatter. Internal links switch articles only within Coffee Note; external reference sites continue to open in the system's default browser.
