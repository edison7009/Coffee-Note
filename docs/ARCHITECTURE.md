...
Agent 使用小型自研 Rust 核心（`agent_loop.rs` / `agent_tools.rs`），
只实现 Coffee Note 必需的能力：工具调用循环、本地知识检索、笔记读写与
记忆路由。不嵌入外部 Agent 运行时，不开放任意 Shell 或任意文件写入。
...
每次 Agent 请求由轻量路由器组合三类上下文：当前问题相关的「我的资料」
摘要、当前笔记目录的 Library Graph 命中，以及最近对话。个人资料检索
保持在约 16 KB；当前笔记只注入相关片段，并受应用级上下文维护上限约
1 MB 约束。这个预算不会作为「省钱模式」削减任务轮次；Agent 会继续工作
直到完成、用户取消、供应商失败或触发全局异常安全上限。系统提示本身
保持稳定，动态资料放入用户消息，从而保留 DeepSeek 前缀缓存复用的机会。
