...
每次 Agent 请求由轻量路由器组合三类上下文：当前问题相关的“我的资料”摘要、当前
笔记目录的 Library Graph 命中，以及最近对话。个人资料检索保持在约 16 KB；当前
笔记只注入相关片段，并受应用级上下文维护上限约 1 MB 约束。这个预算不会作为
“省钱模式”削减任务轮次；Agent 会继续工作直到完成、用户取消、供应商失败或触发
全局异常安全上限。系统提示本身保持稳定，动态资料放入用户消息，从而保留 DeepSeek
前缀缓存复用的机会。
