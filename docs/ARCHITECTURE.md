...
每次 Agent 请求由轻量路由器按固定预算组合三类上下文：当前问题相关的“我的资料”
摘要、当前笔记目录的 Library Graph 命中，以及最近对话。省钱模式把个人资料控制在
约 8 KB、当前笔记控制在约 24 KB；完整模式才放宽笔记预算。系统提示本身保持稳定，
动态资料放入用户消息，从而保留 DeepSeek 前缀缓存复用的机会。
