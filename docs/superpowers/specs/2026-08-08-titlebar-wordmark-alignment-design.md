...
Use the cleaner desktop shell shown in the approved reference: remove the duplicate feather mark from the custom title bar and align the `TierNote` wordmark's left edge with the Home row icon in the navigation rail.

## Design

- `AppTitlebar` renders only the `TierNote` wordmark inside `.titlebar-brand`; history controls and menus keep their existing behavior and order.
...
- `TierNote` remains visible in the title bar and uses the existing Lora wordmark style.
